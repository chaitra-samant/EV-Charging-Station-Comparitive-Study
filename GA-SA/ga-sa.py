"""
Main Hybrid Genetic Algorithm + Simulated Annealing implementation for EV Charging Station Location
"""
...
from config import GA_PARAMS, SA_PARAMS
import os
import warnings
warnings.filterwarnings('ignore')

def hybrid_ga_sa(candidate_coords, demand_coords, demand_weights, p, verbose=True):
    """
    Main Hybrid GA-SA loop
    ...
    for generation in range(max_iter):
        ...
        # Apply SA to some individuals (local search)
        for i in range(pop_size):
            if np.random.rand() < apply_sa_prob:
...
if __name__ == "__main__":
    ...
    print("Hybrid Genetic Algorithm + Simulated Annealing Implementation")
    print("\n\n\n")
    ...
    # Run Hybrid for multiple p values
    print(f"\n\n\n")
    ...
    for p in GA_PARAMS['num_stations']:
        print(f"\n{'='*60}")
        print(f"Running Hybrid GA-SA for p={p} stations")
        print(f"{'='*60}")
        
        best_sol, best_cost, history, metrics = hybrid_ga_sa(
