...
def plot_delhi_map_enhanced(selected_stations_df, all_stations_df, p, 
    ...
    <div style="position: fixed; top: 10px; left: 50px; width: 420px; 
        ...
        ⚡ EV Charging Station Optimization - Delhi (p={p})
    </div>
    '''
...
def plot_convergence_single(convergence_data, p, output_path='results/plots'):
    ...
    plt.title(f'PSO Convergence - p={p} Stations', fontsize=14, fontweight='bold')
...
def plot_cost_comparison(results_df, output_path='results/plots'):
    ...
    ax1.set_title('Total Distance vs Stations', fontsize=13, fontweight='bold')
    ...
    ax2.set_title('Average Customer Distance vs Stations', fontsize=13, fontweight='bold')
...
def create_visualizations(results_csv='results/pso_results.csv',
    ...
    if not os.path.exists(results_csv):
        print("Results not found. Run particle_swarm_optimization.py first.")
        return
